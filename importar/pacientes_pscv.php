...
<div id="demos">
    ...
    <hr />
    <form name="frmload" method="post" action="pacientes_pscv.php" enctype="multipart/form-data">
                        ...
                        if(valida_rut($rut)==true){
                            ...
                            if($persona->existe==true){
                                $sql1 = "update persona set 
                                          nombre_completo=upper('$nombre'),
                                          telefono=upper('$telefono'),
                                          email=upper('$email'),
                                          fecha_nacimiento=upper('$nacimiento'),
                                          sexo=upper('$sexo'),
                                          direccion=upper('$direccion'),
                                          comuna=upper('$comuna'),
                                          carpeta_familiar=upper('$carpeta'),
                                          numero_ficha=upper('$ficha')
                                          where rut='$rut' ";
                                mysql_query($sql1);
                                $error .='<div style="padding: 5px;background-color: orange;margin-bottom: 2px;border: solid 2px orange;">EL RUT SE ACTUALIZO SEGUN EL NUEVO REGISTRO EN LA FILA '.$v.', NO SE PUEDE REGISTRAR</div>';
                            }else{
                            ...
                            $sql2 = "select * from paciente_establecimiento where rut='$rut' limit 1";
                            $row2 = mysql_fetch_array(mysql_query($sql2));
                            if($row2){
                                $sql2 = "update paciente_establecimiento 
                                            set m_cardiovascular='SI',id_sector='$sector'  
                                            where rut='$rut'";
                                mysql_query($sql2);
                            }else{
                                $sql2 = "insert into paciente_establecimiento(rut,id_establecimiento,id_sector,m_cardiovascular) 
                                  values('$rut','$id_establecimiento','$sector','SI')";
                                mysql_query($sql2);
                            }


                            $sql3 = "insert into paciente_pscv(rut) 
                                  values('$rut')";
                            mysql_query($sql3);
